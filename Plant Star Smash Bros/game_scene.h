...
public:
	...
	void on_enter()
	{
		...
		pos_img_draw_text.x = pos_img_winner_bar.x;
		pos_img_draw_text.y = (getheight() - img_draw_player.getheight()) / 2;
		pos_x_img_draw_text_dst = (getwidth() - img_draw_player.getwidth()) / 2;
	...
	void on_update(int delta)
	{
		...
		if (is_game_over)
		{
			...
			pos_img_draw_text.x += (int)(speed_draw_text * delta);
			...
			if (!is_slide_out_started)
			{
				...
				if (player_1->get_hp() <= 0 && player_2->get_hp() <= 0)
				{
					if (pos_img_draw_text.x > pos_x_img_draw_text_dst)
						pos_img_draw_text.x = pos_x_img_draw_text_dst;
				}
				else
				{
	...
	void on_draw(const Camera& camera)
	{
		...
		if (is_game_over)
		{
			...
			if (player_1->get_hp() <= 0 && player_2->get_hp() <= 0)
				putimage_alpha(pos_img_draw_text.x, pos_img_draw_text.y, &img_draw_player);
			else
			{
...
private:
	...
	POINT pos_img_winner_text = { 0 };  // 获胜结算动效文本位置
	POINT pos_img_draw_text = { 0 };    // 平局结算动效文本位置
	int pos_x_img_winner_bar_dst = 0;   // 结算动效背景移动的目标位置
	int pos_x_img_winner_text_dst = 0;  // 获胜结算动效文本移动的目标位置
	int pos_x_img_draw_text_dst = 0;    // 平局结算动效文本移动的目标位置
